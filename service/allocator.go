...
func (s *allocatorService) GetIdRange(req *model.ApplyReq) (int64, int64, error) {
	...
	if entity == nil {
		...
		if err != nil {
			//出错
			logs.Error("{} {} {} 查询更新后的号段失败 err: {} 请求: {}, 进新更新的数据: {}", trace.GetGoroutineID(), req.AppName, req.BizType, err.Error(), req, newEntity)
			return 0, 0, err
		}
		logs.Debug("{} {} {} 当天第一个号段 请求:{} 返回号段: {} {}， 更新后号段记录: {}", trace.GetGoroutineID(), req.AppName, req.BizType, req, rangeStart, rangeEnd, entityAfterUpdated)
		return rangeStart, rangeEnd, nil
	}

	logs.Debug("{} {} {} 当天非第一次申请，原号段记录: {}, 新请求: {}", trace.GetGoroutineID(), req.AppName, req.BizType, entity, req)
	//更新对应申请日期使用的号段
	...
	if err != nil {
		//出错
		logs.Error("{} {} {} 查询更新后的号段失败 err: {} 请求: {}, 更新数据: {}", trace.GetGoroutineID(), req.AppName, req.BizType, err.Error(), req, entity)
		return 0, 0, err
	}

	logs.Debug("{} {} {} 当天非第一次申请结果，返回新号段:{} {}, 更新数据:{}, 更新后记录:{}", trace.GetGoroutineID(), req.AppName, req.BizType, rangeStart, rangeEnd, entity, entityAfterUpdated)
